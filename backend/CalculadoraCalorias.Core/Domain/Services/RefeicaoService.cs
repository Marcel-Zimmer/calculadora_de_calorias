...
{
    public class RefeicaoService(IRefeicaoRepository _refeicaoRepository) : IRefeicaoService
    {
        public async Task<Refeicao> Adicionar(long usuarioId, string? apelido, double peso, TipoRefeicaoEnum tipo, DateOnly data, Guid guidArquivo)
        {
        ...
        public async Task<Refeicao> AdicionarManual(long usuarioId, string? apelido, string alimento, int calorias, double proteinas, double carboidratos, double gorduras, double acucares, double fibras, double peso, TipoRefeicaoEnum tipo, DateOnly data)
        {
        ...
        public async Task<Refeicao?> AdicionarBaseadoEmModelo(long usuarioId, long modeloId, double pesoEmGramas, TipoRefeicaoEnum tipo, DateOnly data)
        {
            var modelo = await _refeicaoRepository.ObterPorId(modeloId);
            if (modelo == null) return null;

            double fator = pesoEmGramas / modelo.Peso;
